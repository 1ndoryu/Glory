...
class EmailSignupService
{
    ...
    public static function registerAjaxHooks(): void
    {
        ...
        add_action('wp_ajax_'        . self::ACTION_REGISTER, [self::class, 'handleRegistration']); // Also allow logged-in if needed?

        // Hook for updating user details (could potentially be restricted to logged-in later)
        // For now, assuming it can happen immediately after registration (non-logged-in context initially)
        add_action('wp_ajax_nopriv_' . self::ACTION_UPDATE, [self::class, 'handleDetailsUpdate']);
        add_action('wp_ajax_'        . self::ACTION_UPDATE, [self::class, 'handleDetailsUpdate']); // Allow logged-in users to update too
    }
