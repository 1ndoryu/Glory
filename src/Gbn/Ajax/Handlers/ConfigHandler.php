...
class ConfigHandler
{
    public static function saveConfig(): void
    {
        ...
        $context = isset($_POST['context']) ? sanitize_key($_POST['context']) : 'post';
        ...
        if ($context === 'header' || $context === 'footer') {
            if (!current_user_can('edit_theme_options')) {
                wp_send_json_error(['message' => 'Sin permisos para editar temas']);
            }
        } else {
            if (!$pageId || !current_user_can('edit_post', $pageId)) {
                wp_send_json_error(['message' => 'Sin permisos o ID inválido']);
            }
        }
        ...
        // --- PERSISTENCIA DEPENDIENDO DEL CONTEXTO ---
        $mode = 'editor'; 
        $manualEditDetected = false; 
        $contentUpdated = false;

        if ($context === 'header') {
            TemplateService::saveHeaderConfig($configById);
            TemplateService::saveHeaderStyles($stylesById);
            
            // Guardar CSS Responsive (Opcional: Podríamos guardarlo en otra option si es necesario)
            // Header NO soporta 'code' mode por ahora, siempre es visual.

            // Guardar HTML
            if (isset($_POST['htmlContent'])) {
                $html = wp_unslash($_POST['htmlContent']);
                $html = DomProcessor::processHtmlForPersistence($html, $configById);
                TemplateService::saveHeaderContent($html);
                $contentUpdated = true;
            }
            
            Logger::log('[saveConfig] Saved Header Template');

        } elseif ($context === 'footer') {
            TemplateService::saveFooterConfig($configById);
            TemplateService::saveFooterStyles($stylesById);

            // Guardar HTML
            if (isset($_POST['htmlContent'])) {
                $html = wp_unslash($_POST['htmlContent']);
                $html = DomProcessor::processHtmlForPersistence($html, $configById);
                TemplateService::saveFooterContent($html);
                $contentUpdated = true;
            }

            Logger::log('[saveConfig] Saved Footer Template');

        } else {
            // --- LÓGICA ORIGINAL PARA POSTS ---
