...
class FooterEditorPage
{
    ...
    {
        if ($hook !== 'appearance_page_gbn-edit-footer') {
            return;
        }
        ...
        // Scripts - List identical to HeaderEditorPage
            $frontendScripts = [
                'glory-gbn-core' => [
        ...
        // Localize for FOOTER
        $presets = [
            'config' => TemplateService::getFooterConfig(),
            'styles' => TemplateService::getFooterStyles(),
        ];
        ...
        $localizedData = [
            ...
            'initialActive' => false,
            'pageId' => 0,
            'context' => 'footer', // VITAL for persistence
    ...
    {
        $content = TemplateService::getFooterContent();
        ?>
        <div class="wrap gbn-editor-page">
            <h1 class="wp-heading-inline">Gestor de Pie de Página (Footer)</h1>
            <p>Edita el diseño del footer que se aplicará en todo el sitio.</p>
            ...
            <div class="gbn-actions" style="margin-top: 20px; text-align: right;">
                 <button id="gbn-manual-save-btn" class="button button-primary button-large">Guardar Footer</button>
            </div>
        </div>
            ...
            $('#gbn-manual-save-btn').on('click', function() {
                ...
                if(Gbn && Gbn.persistence) {
                    Gbn.persistence.savePageConfig().then(function(res) {
                        btn.prop('disabled', false).text('Guardar Footer');
                        if(res.success) {
