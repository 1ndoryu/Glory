(function (global) {
    ...
    function computeTree(blocks) {
        ...
        list.forEach(function (b) {
            ...
            if (parent) {
                childrenMap[parent.id].push(b.id);
            } else {
        ...
        function assignOrder(ids) {
            return sortByDom(ids).map(function (id, i) {
                return {id: id, order: i};
            });
        }
    ...
    function cleanPostRenderClones(container) {
        ...
        postRenders.forEach(function (pr) {
            ...
            previewFields.forEach(function (field) {
                ...
                // BUG-019 FIX: Restaurar el style original si fue modificado
                // Los campos featuredImage con asBackground agregan background-image dinamicamente
                var originalStyle = field.getAttribute('data-gbn-original-style');
                if (originalStyle !== null) {
                    if (originalStyle === '') {
                        field.removeAttribute('style');
                    } else {
                        field.setAttribute('style', originalStyle);
                    }
                }

                // Limpiar atributos de preview
                field.removeAttribute('data-gbn-original-content');
                field.removeAttribute('data-gbn-original-style');
    ...
    function collectBlocksPayload() {
        ...
        var orderById = {};
        tree.root.forEach(function (x) {
            orderById[x.id] = x.order;
        });
        Object.keys(tree.childrenMap).forEach(function (pid) {
            (tree.childrenMap[pid] || []).forEach(function (x) {
                orderById[x.id] = x.order;
            });
        });
        ...
        return blocks.map(function (b) {
            ...
            return {
                ...
                styles: styles,
                children: (tree.childrenMap[b.id] || []).map(function (x) {
                    return x.id;
                }),
    ...
    async function savePageConfig() {
        ...
        var responsiveCss = Gbn.services && Gbn.services.styleGenerator ? Gbn.services.styleGenerator.generateCss(blocksMap) : '';
