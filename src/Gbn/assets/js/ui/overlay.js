;(function (global) {
    ...
    function parseSpacingValue(raw, fallbackUnit) {
        ...
        return {
            valor: match[1],
            unidad: match[2] || fallbackUnit || 'px'
        };
    }
    ...
    var styleResolvers = {
        principal: function (config) {
            ...
            if (config.alineacion && config.alineacion !== 'inherit') {
            ...
            if (config.fondo) {
                styles.background = config.fondo;
            }
            return styles;
        },
    ...
    function buildSpacingField(block, field) {
        ...
        function handleSpacingInput(event) {
            ...
            var finalValue = value === '' ? null : value + unit;
        ...
        unitSelect.addEventListener('change', function () {
            ...
            inputs.forEach(function (input) {
                ...
                if (input.value === '') {
                    updateConfigValue(activeBlock, input.dataset.configPath, null);
                } else {
                    updateConfigValue(activeBlock, input.dataset.configPath, input.value + unitSelect.value);
                }
            });
        });
    ...
    function buildSliderField(block, field) {
        ...
        if (current === null || current === undefined || current === '') {
            current = field.min !== undefined ? field.min : 0;
        }
        input.value = current;
        valueBadge.textContent = input.value + (field.unidad ? field.unidad : '');
    ...
    function buildSelectField(block, field) {
        ...
        select.addEventListener('change', function () {
            updateConfigValue(activeBlock, field.id, select.value);
        });
    ...
    function buildToggleField(block, field) {
        ...
        wrapper.className = 'gbn-field gbn-field-toggle';
        var input = document.createElement('input');
        input.type = 'checkbox';
        input.className = 'gbn-toggle';
        var current = !!getConfigValue(block, field.id);
        input.checked = current;
        input.addEventListener('change', function () {
            updateConfigValue(activeBlock, field.id, !!input.checked);
        });
        var span = document.createElement('span');
        span.textContent = field.etiqueta || field.id;
        wrapper.appendChild(input);
    ...
    function buildTextField(block, field) {
        ...
        input.addEventListener('input', function () {
            updateConfigValue(activeBlock, field.id, input.value);
        });
        wrapper.appendChild(input);
        appendFieldDescription(wrapper, field);
        return wrapper;
    ...
    function buildColorField(block, field) {
        ...
        var input = document.createElement('input');
        input.type = 'color';
        input.className = 'gbn-input-color';
        var current = getConfigValue(block, field.id);
    ...
    function buildField(block, field) {
        ...
        switch (field.tipo) {
            case 'spacing':
                return buildSpacingField(block, field);
            case 'slider':
                return buildSliderField(block, field);
            case 'select':
                return buildSelectField(block, field);
            case 'toggle':
                return buildToggleField(block, field);
            case 'color':
                return buildColorField(block, field);
            case 'text':
            default:
                return buildTextField(block, field);
        }
    }
    ...
    function renderBlockControls(block) {
        ...
        panelBody.innerHTML = '';
        panelForm = null;
        panelBody.appendChild(createSummary(block));
    ...
    var panel = {
        ...
        openTheme: function () {
            ...
            if (panelTitle) {
                panelTitle.textContent = 'Theme settings';
            }
            if (panelBody) {
                panelBody.innerHTML = '<div class="gbn-panel-empty">La configuración global del tema estará disponible en la etapa de configuraciones.</div>';
                panelForm = null;
            }
        ...
        openPage: function () {
            ...
            if (panelBody) {
                panelBody.innerHTML = '<div class="gbn-panel-empty">Define padding, fondo y opciones por página en la siguiente fase.</div>';
                panelForm = null;
            }
            setPanelStatus('Próximamente');
            if (panelFooter) {
        ...
        openRestore: function () {
            ...
            if (panelTitle) {
                panelTitle.textContent = 'Restaurar valores';
            }
            if (panelBody) {
                panelBody.innerHTML = '<div class="gbn-panel-empty">La restauración devolverá el marcado original escrito en código. Esta acción se conectará en la etapa de persistencia.</div>';
                panelForm = null;
            }
            setPanelStatus('Próximamente');
            if (panelFooter) {
