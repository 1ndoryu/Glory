;(function(global) {
    ...
    var resolvers = {
        ...
        principal: function(config, block) {
            return Gbn.ui.renderers.principal 
                ? Gbn.ui.renderers.principal.getStyles(config, block) 
                : {};
        },
        secundario: function(config, block) {
            return Gbn.ui.renderers.secundario 
                ? Gbn.ui.renderers.secundario.getStyles(config, block) 
                : {};
        },
        content: function() { 
            return {}; 
        ...
        text: function(config, block) { 
            return Gbn.ui.renderers.text 
                ? Gbn.ui.renderers.text.getStyles(config, block) 
                : {};
        },
        button: function(config, block) {
            return Gbn.ui.renderers.button 
                ? Gbn.ui.renderers.button.getStyles(config, block) 
                : {};
        },
        image: function(config, block) {
            return Gbn.ui.renderers.image 
                ? Gbn.ui.renderers.image.getStyles(config, block) 
                : {};
        },
        
        // Fase 13: PostRender - Contenido Dinámico
        postRender: function(config, block) {
            return Gbn.ui.renderers.postRender 
                ? Gbn.ui.renderers.postRender.getStyles(config, block) 
                : {};
        },
        postItem: function(config, block) {
            return Gbn.ui.renderers.postItem 
                ? Gbn.ui.renderers.postItem.getStyles(config, block) 
                : {};
        },
        postField: function(config, block) {
            return Gbn.ui.renderers.postField 
                ? Gbn.ui.renderers.postField.getStyles(config, block) 
                : {};
        },
        
        // Fase 14: Form Components
        form: function(config, block) {
            return Gbn.ui.renderers.form 
                ? Gbn.ui.renderers.form.getStyles(config, block) 
                : {};
        },
        input: function(config, block) {
            return Gbn.ui.renderers.input 
                ? Gbn.ui.renderers.input.getStyles(config, block) 
                : {};
        },
        textarea: function(config, block) {
            return Gbn.ui.renderers.textarea 
                ? Gbn.ui.renderers.textarea.getStyles(config, block) 
                : {};
        },
        select: function(config, block) {
            return Gbn.ui.renderers.select 
                ? Gbn.ui.renderers.select.getStyles(config, block) 
                : {};
        },
        submit: function(config, block) {
            return Gbn.ui.renderers.submit 
                ? Gbn.ui.renderers.submit.getStyles(config, block) 
                : {};
        },
        
        // Fase 15: Layout Components (Header, Footer, Menu, Logo)
        header: function(config, block) {
            return Gbn.ui.renderers.header 
                ? Gbn.ui.renderers.header.getStyles(config, block) 
                : {};
        },
        logo: function(config, block) {
            return Gbn.ui.renderers.logo 
                ? Gbn.ui.renderers.logo.getStyles(config, block) 
                : {};
        },
        menu: function(config, block) {
            return Gbn.ui.renderers.menu 
                ? Gbn.ui.renderers.menu.getStyles(config, block) 
                : {};
        },
        footer: function(config, block) {
            return Gbn.ui.renderers.footer 
                ? Gbn.ui.renderers.footer.getStyles(config, block) 
                : {};
        },
        menuItem: function(config, block) {
            return Gbn.ui.renderers.menuItem 
                ? Gbn.ui.renderers.menuItem.getStyles(config, block) 
                : {};
        }
    };
    ...
    function getResolver(role) {
        return resolvers[role] || function() { return {}; };
    }
     ...
     */
    var statesSupportedRoles = [
        'principal', 'secundario', 'text', 'button', 'image',
        'postRender', 'postItem', 'postField',
        'form', 'input', 'textarea', 'select', 'submit',
        'header', 'logo', 'menu', 'footer', 'menuItem'
    ];
