(function (global) {
    ...
    /**
     * BUG-021 FIX: Aplica SOLO una variable CSS especifica
     * Esta funcion evita inyectar +100 variables de defaults cuando el usuario
     * edita UN solo valor en el panel de Theme Settings.
     *
     * @param {string} path - Path del valor (ej: "components.principal.padding.superior")
     * @param {*} value - Valor a aplicar
     * @param {string} breakpoint - Breakpoint actual (desktop, tablet, mobile)
     */
    function applySingleValue(path, value, breakpoint) {
        var root = document.querySelector('[data-gbn-root]') || document.documentElement;
        if (!path || value === undefined) return;

        var segments = path.split('.');

        // Manejar paths de componentes: "components.role.property" o "components.role.property.subprop"
        if (segments[0] === 'components' && segments.length >= 3) {
            var role = segments[1];
            var fieldId = segments[2];
            var subProp = segments.length > 3 ? segments[3] : null;

            var prefix = '--gbn-' + role;

            // Manejar propiedades de spacing (padding, margin)
            if (subProp && (fieldId === 'padding' || fieldId === 'margin')) {
                var map = {superior: 'top', derecha: 'right', inferior: 'bottom', izquierda: 'left'};
                var cssDir = map[subProp] || subProp;
                var varName = prefix + '-' + fieldId + '-' + cssDir;

                if (value !== null && value !== undefined && value !== '') {
                    root.style.setProperty(varName, toCssValue(value));
                } else {
            ...
            // Manejar propiedades simples
            var kebabId = fieldId.replace(/([a-z0-9]|(?=[A-Z]))([A-Z])/g, '$1-$2').toLowerCase();
            var varName = prefix + '-' + kebabId;

            // Casos especiales de mapeo
            if (fieldId === 'maxAncho') varName = prefix + '-max-width';
            if (fieldId === 'fondo') varName = prefix + '-background';
            if (fieldId === 'layout') varName = prefix + '-display';
            if (fieldId === 'gridColumns') varName = prefix + '-grid-columns';

            // GridColumns es unitless
            if (fieldId === 'gridColumns') {
                if (value !== null && value !== undefined && value !== '') {
        ...
        // Manejar paths de colores: "colors.primary", "colors.secondary", etc
        if (segments[0] === 'colors' && segments.length >= 2) {
            var colorName = segments[1];
            var cssMap = {
                primary: '--gbn-primary',
                secondary: '--gbn-secondary',
                accent: '--gbn-accent',
                background: '--gbn-bg'
            };
            var varName = cssMap[colorName] || '--gbn-' + colorName;
        ...
        // Fallback para otros paths: aplicar como variable generica
        if (Gbn.log) {
            Gbn.log.info('applySingleValue fallback', {path: path, value: value});
        }
    }
    ...
    Gbn.ui.theme.applicator = {
        ...
        applySingleValue: applySingleValue
    };
