(function (global) {
    ...
    function renderThemeSettingsForm(settings, container, footer) {
        ...
        // BUG-021 FIX: Guardar copia de los settings ORIGINALES (solo valores guardados por usuario)
        // Esto es lo que se usara para inyectar variables CSS - NO incluye defaults inferidos
        var savedSettings = settings ? JSON.parse(JSON.stringify(settings)) : {};
        ...
        // BUG-021 FIX: El merge de defaults solo es para UI (mostrar valores en el panel)
        // NO afecta a savedSettings que se usa para inyectar CSS
        ...
        // Sincronización con CSS en modo dev - solo afecta UI, no savedSettings
        ...
        // BUG-021 FIX: Aplicar tema con SOLO los valores guardados, NO los defaults inferidos
        // Esto evita inyectar +100 variables CSS innecesarias en el root
        if (Gbn.ui.theme.applicator && Gbn.ui.theme.applicator.applyThemeSettings) {
            // Usar savedSettings (valores originales) en lugar de mockBlock.config (con defaults)
            Gbn.ui.theme.applicator.applyThemeSettings(savedSettings);

            if (Gbn.log) {
                Gbn.log.info('Theme Settings Applied (only saved values)', {
                    view: state.getCurrentView(),
                    savedKeys: Object.keys(savedSettings),
                    uiConfigKeys: Object.keys(mockBlock.config)
                });
            }
        }
