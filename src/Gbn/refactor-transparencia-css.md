...
| 2025-12-08 | **FIX: Estilos compartidos cases** - Agregados selectores compuestos (.landing-container, .casos-container) para .cases-grid y relacionados. Agregado .casos-container a container-type en init.css. Container queries explicitas para responsive en constructor.                                                                                                                                               | IA    |
| 2025-12-08 | **FIX: Container Queries solapadas** - Corregidas queries en landing.css y casos.css. Tablet ahora usa rango `(min-width: 769px) and (max-width: 1024px)` para no solaparse con movil `(max-width: 768px)`. Esto asegura 1 columna en movil, 2 en tablet, 3 en desktop.                                                                                                                                         | IA    |
| 2025-12-08 | **FIX: PostRender inline styles** - Agregado `!important` a propiedades grid de `.cases-grid` en container queries. Esto sobrescribe los estilos inline que el renderer de PostRender aplica, permitiendo que el CSS responsive del tema funcione en el constructor de GBN. NOTA: Solucion temporal; la arquitectura ideal es que el renderer detecte clases CSS personalizadas y no aplique estilos de layout. | IA    |
